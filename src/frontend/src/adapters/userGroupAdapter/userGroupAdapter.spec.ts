...
import { adaptUserGroupsForOptions } from "./userGroupAdapter";
...
describe("adaptUserGroupsForOptions", () => {
	...
	it("should return empty list if input is undefined", () => {
		...
		const value = adaptUserGroupsForOptions(undefined);
		expect(value).toStrictEqual([]);
	});

	it("should return empty list if input is null", () => {
		// @ts-expect-error accepts array
		const value = adaptUserGroupsForOptions(null);
		expect(value).toStrictEqual([]);
	});

	it("should return empty list if input is not an array but has length", () => {
		// @ts-expect-error accepts array
		const value = adaptUserGroupsForOptions("test input");
		expect(value).toStrictEqual([]);
	});
	...
	it("should return the correct format", () => {
		...
		const value = adaptUserGroupsForOptions(userGroupsResponse);
