...
def test_delete_producer_updates_instance(producer, test_user):
...
def test_delete_producer_missing_delete_by_does_not_update_instance(
    producer,
):
...
def test_delete_producer_missing_instance_does_not_update_instance(
    test_user,
):
    with pytest.raises(AttributeError):
        delete_producer(
