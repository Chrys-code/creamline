...
def milk_interval_comparison_data():
    ...
    # Current periods
    last_week_qs = qs.filter(created_at__date__gte=this_week_start)
    last_month_qs = qs.filter(created_at__date__gte=this_month_start)
    last_year_qs = qs.filter(created_at__date__gte=this_year_start)
    ...
    # SUMS
    week_total = _sum_liters(last_week_qs)
    prev_week_total = _sum_liters(prev_week_qs)

    month_total = _sum_liters(last_month_qs)
    prev_month_total = _sum_liters(prev_month_qs)

    year_total = _sum_liters(last_year_qs)
